...
namespace Osaro.Utilities
{
    public class MeshUtils
    {
        ...
        private static Quaternion GetQuaternionEuler(float rotationFloat)
        {
            ...
            //if (rot >= 360) rot -= 360;
            if (cachedQuaternionEulerArray == null) CachedQuaternionEuler();
        ...
        {
            //int quadCount: The number of quads(rectangles) you want to create arrays for
            //4 * quadCount because each quad(rectangle) requires 4 vertices.
            vertices = new Vector3[4 * quadCount];
            uv = new Vector2[4 * quadCount];
            //because each quad is composed of 2 triangles, and each triangle is defined by 3 indices(2 triangles * 3 indices = 6)
            triangles = new int[6 * quadCount];
        }

        public static Mesh CreateMesh(Vector2 pos, float rotation, Vector3 baseSize, Vector2 uv00,Vector2 uv11)
        {
            return AddToMesh(null, pos, rotation, baseSize, uv00, uv11);
        }

        public static Mesh AddToMesh(Mesh mesh, Vector3 pos, float rotation, Vector3 baseSize, Vector2 uv00, Vector2 uv11)
        {
            if(mesh == null)
            {
            ...
            int[] triangles = new int[4 + mesh.triangles.Length];

            // coping the existing value
            ...
            //The base size is halved to get the offsets from the center of the quad.
            baseSize *= .5f;

            // line checks if the quad is not a perfect square
            ...
            //Relocate UVs
            uvs[vIndex0] = new Vector2(uv00.x, uv11.y);   // (0, 1)
            uvs[vIndex1] = new Vector2(uv00.x, uv00.y);   // (0, 0)
            uvs[vIndex2] = new Vector2(uv11.x, uv00.y);   // (1, 0)
            uvs[vIndex3] = new Vector2(uv11.x, uv11.y);   // (1, 1)
